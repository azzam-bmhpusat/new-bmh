...
export default function Navv() {
  ...
  // Cek prefix URL untuk nyalain induk
  const isProfilActive = path.startsWith('/profil-bmh') || path.startsWith('/jajaran-manajemen') || path.startsWith('/kantor-perwakilan');
  const isLayananActive = path.startsWith('/kalkulator-zakat') || path.startsWith('/jemput-zakat');
  const isPublikasiActive = path.startsWith('/majalah-mulia') || path.startsWith('/under-construction') || path.startsWith('/riset-pengembangan') || path.startsWith('/laporan-keuangan') || path.startsWith('/laporan-tahunan');
      ...
      <nav aria-label="Global" className="flex items-center justify-between p-3 px-4 mx-auto max-w-7xl md:px-6 lg:px-8 ">
        ...
        <PopoverGroup className="hidden lg:flex lg:gap-x-12">
          ...
          <Popover className="relative">
            <PopoverButton className={`flex items-center gap-x-1 font-semibold text-sm/6 transition-all duration-200 ${isProfilActive ? 'text-[#39BA87]' : 'text-[#095046] hover:text-[#39BA87]'}`}>
              Profil
              <ChevronDownIcon className="w-5 h-5 text-gray-400" />
            </PopoverButton>
            <PopoverPanel className="absolute z-10 w-64 mt-3 bg-white shadow-lg rounded-2xl">
              <div className="p-4">
                {profil.map((item) => (
          ...
          <Popover className="relative">
            <PopoverButton className={`flex items-center gap-x-1 font-semibold text-sm/6 transition-all duration-200 ${isLayananActive ? 'text-[#39BA87]' : 'text-[#095046] hover:text-[#39BA87]'}`}>
              Layanan
              <ChevronDownIcon className="w-5 h-5 text-gray-400" />
            </PopoverButton>
            <PopoverPanel className="absolute z-10 w-64 mt-3 bg-white shadow-lg rounded-2xl">
          ...
          <Popover className="relative">
            <PopoverButton className={`flex items-center gap-x-1 font-semibold text-sm/6 transition-all duration-200 ${isPublikasiActive ? 'text-[#39BA87]' : 'text-[#095046] hover:text-[#39BA87]'}`}>
              Publikasi
              <ChevronDownIcon className="w-5 h-5 text-gray-400" />
            </PopoverButton>
            <PopoverPanel className="absolute z-10 w-64 mt-3 bg-white shadow-lg rounded-2xl">
